...
function IconCode({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 64 64" fill="none" aria-hidden>
      ...
      <path
        d="M24 28l-6 6 6 6M40 28l6 6-6 6M34 24l-4 16"
        stroke="#93C5FD"
        strokeWidth="2.5"
        strokeLinecap="round"
      />
...
const devLinks = [
  ...
  { name: 'Base64 Encoder', href: '/dev-tools/base64-encoder' },
  { name: 'Password Generator', href: '/dev-tools/password-generator' },
] as const
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 antialiased">
      <header className="sticky top-0 z-50 border-b border-slate-100/80 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-6 px-4 py-4 sm:px-6 lg:px-8">
          <a href="/" className="shrink-0 text-xl font-bold tracking-tight">
            <span className="text-slate-900">Online Tools</span> <span className="text-blue-600">Hub</span>
          </a>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-xl border border-slate-100 bg-white p-8 shadow-md shadow-slate-200/50">
            ...
            <ul className="mt-6 space-y-3">
              {financialLinks.map(({ name, href }) => (
                <li key={name}>
                  <a href={href} className="flex items-center gap-2 text-slate-700 transition hover:text-blue-600">
      ...
      <section className="border-t border-slate-100 bg-white px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl text-center">
          <h2 className="text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Get Started with Our Free Tools</h2>
          <a
            href="/tools"
